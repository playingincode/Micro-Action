...
class TemporalShift(nn.Module):
    ...
    def __init__(self, net, num_segments=8, shift_div=8):
        ...
        self.fcce = nn.Sequential(
            nn.Linear(self.in_channels, self.reduced_channels, bias=False),   
            nn.ReLU(inplace=True),
            nn.Linear(self.reduced_channels, self.in_channels, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x_shift=x
        b, c, _, _ = x_shift.size()
        y = self.avg_pool(x_shift).view(b, c)     
